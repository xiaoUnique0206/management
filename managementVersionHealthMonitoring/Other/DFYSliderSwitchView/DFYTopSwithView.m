...
@interface DFYTopSwithView ()

// 当TabBar支持滚动时，使用此scrollView
@property (nonatomic, strong) UIScrollView *scrollView;

@property (nonatomic, strong) DFYSwithItem *otherItem;

@property (nonatomic, strong) UIView *otherLien;

@property (nonatomic, copy) void (^otherItemHandler)(DFYSwithItem *item);
...
@implementation DFYTopSwithView
...
- (void)setItems:(NSArray *)items {
    ...
    // 初始化每一个item
    for (DFYSwithItem *item in self.items) {
...
- (void)setTitles:(NSArray *)titles {
    ...
    for (NSString *title in titles) {
        DFYSwithItem *item = [[DFYSwithItem alloc] init];
        item.title = title;
...
- (void)updateItemsFrame {
    ...
    if (_itemFullWidth) {
        ...
        for (NSInteger index = 0; index < self.items.count; index++) {
            DFYSwithItem *item = self.items[index];
...
//    }
    ...
    if (_selectedItemIndex >= 0) {
        DFYSwithItem *oldSelectedItem = self.items[_selectedItemIndex];
    ...
    DFYSwithItem *newSelectedItem = self.items[selectedItemIndex];
...
- (void)tabItemClicked:(DFYSwithItem *)item {
    
    self.selectedItemIndex = item.index;
}

- (void)otherItemClicked:(DFYSwithItem *)item {
...
// 设置一个单独的item
- (void)setOtherItem:(DFYSwithItem *)item tapHandler:(void (^)(DFYSwithItem *item))handler {
    self.otherItem = item;
    ...
    _otherLien.backgroundColor = [[UIColor whiteColor] colorWithAlphaComponent:0.8];
...
- (void)updateSubViewsWhenParentScrollViewScroll:(UIScrollView *)scrollView {
    ...
    DFYSwithItem *leftItem = self.items[leftIndex];
    DFYSwithItem *rightItem = nil;
